...
요약하자면 TDZ 오류는 `let`/`const` 선언이 스코프 상위로 올리기 때문에 발생하지만, `var`와 달리, 원래 선언이 나타난 코드 시퀀싱의 순간까지 변수의 자동 초기화를 지연합니다. 이 시간 창(힌트: )은 길이가 무엇이든 TDZ이다.
...
변수의 선언 및 재선언은 런타임 작업으로 간주될 때 혼란을 야기하는 경향이 있다. 그러나 이러한 작업에 대한 컴파일 타임 작업으로 사고 방식으로 전환하면, 기이한 점과 *그늘*이 줄어든다.
