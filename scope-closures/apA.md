...
내 반론은 일관성을 선호한다면 `const`가 아닌 `var`로 일관하라는 것이다. `let`은 `const`보다 `var`에 더 가깝다는 것이다. 특히 모든 것을 최상의 스코프로 호이스팅하는 일에서 `var`로 일관성을 택한 터라 더욱 그렇다. `const`를 TDZ의 고유의 대상으로 하, TDZ를 순수하게 하고: 항상 스코프 상위에 상수를 선언함으로써 TDZ를 피하는 것이다. 이게 더 합리적이었을 것 같다.
